# Séance — Architecture du site et SEO

> **Statut :** À démarrer  
> **Prérequis :** préparation MVP2 ; inspection du code / du site en préversion.

## Objectif

Trancher le rôle de Flutter vs `/seo`, les URL, les pages indexables et les conséquences sur le backlog technique.

## Sujets à explorer

- Parité Flutter ↔ pages SEO statiques
- URLs, canonical, redirections, sitemap
- Indexation publique vs état actuel
- Priorisation des items audit P0 liés au SEO / HTML

## Sortie attendue

Architecture retenue + backlog technique priorisé pour l’implémentation.

## Décisions

_À compléter pendant la séance._

## Suivi documentaire

_À compléter en fin de séance (`04_SEO_Performance.md`, `TODO.md`, …)._
